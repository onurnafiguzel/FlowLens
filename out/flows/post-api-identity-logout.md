<!-- ÜRETİLMİŞ DOSYA — elle düzenlemeyin. `flowlens docs` ile yeniden üretilir. -->

# POST /api/identity/logout

**Modül:** Identity · **Tanım:** `src/Modules/Identity/ModularCommerce.Identity.Api/Endpoints/AuthEndpoints.cs:52`

```mermaid
flowchart TD
  n0["Identity · LogoutHandler.HandleAsync"]
  n1["Identity · RefreshTokenRepository.GetByTokenHashAsync"]
  n2[["Identity · POST /api/identity/logout"]]
  n3[("Identity · identity.refresh_tokens")]

  n0 -->|"1"| n1
  n0 ==>|"2"| n3
  n1 ==>|"RefreshToken"| n3
  n2 --> n0
```


> **Numaralar kaynak kodda yazılma sırasıdır**, çalışma sırası değil —
> koşullu dallar, döngüler ve erken `return`'ler ikisini ayırır.
> **`koşullu` işaretli adımlar hiç koşmayabilir**, ve bir `if`/ternary'nin iki
> dalındaki adımlar birbirini dışlar — ikisi birden koşmaz.
> Aynı numarayı taşıyan kutular **tek bir çağrıdan** gelir.

## Çağrı sırası

**LogoutHandler.HandleAsync** — `src/Modules/Identity/ModularCommerce.Identity.Application/Auth/Logout/LogoutHandler.cs:13`

1. `LogoutHandler.cs:27` → `RefreshTokenRepository.GetByTokenHashAsync`
2. `LogoutHandler.cs:33` → `identity.refresh_tokens`

## Veri katmanı

| Tablo | Erişim | Kolonlar | Tanım |
|---|---|---|---|
| `identity.refresh_tokens` | WR | `RevokedAtUtc` | `src/Modules/Identity/ModularCommerce.Identity.Infrastructure/Persistence/Configurations/RefreshTokenConfiguration.cs:11` |

## Diyagram neyi göstermiyor

Gösterilen **4** node; ham yürüyüş **17** node'a ulaşıyor. Gizlenen: 5 ara çağrı, 5 utility, 2 arayüz bildirimi, 1 veriye ulaşmayan dal.

Tam liste: `flowlens trace "POST /api/identity/logout"`

## Bilinen sınırlar

Bu sayfa için kaydedilmiş bir sınır yok.

> Diyagram dar görünüyorsa tıklayarak büyütebilir veya
> [mermaid.live'da açabilirsiniz](https://mermaid.live/edit#pako:eAEBlgFp_-fFwiMVwifCBuMVxuICBuMCA9PT58XCIyXCJ8IG4zXG4gIG4xID09PnxcIlJlZnJlc2hUb2tlblwifCBuM1xuICBuMiAtLT4gbjBcbiIsIm1lcm1haWQiOiJ7XG4gIFwidGhlbWVcIjogXCJkZWZhdWx0XCJcbn0iLCJhdXRvU3luYyI6dHJ1ZSwidXBkYXRlRGlhZ3JhbSI6dHJ1ZX0wj4wE).
